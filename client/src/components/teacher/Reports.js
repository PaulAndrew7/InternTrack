  ...
  const Reports = () => {
    ...
    const fetchInternships = async () => {
      try {
        ...
        const uniqueSections = [...new Set(allInternships.map(item => item['Section']).filter(Boolean))];
        const uniqueTypes = [...new Set(allInternships.map(item => item['Internship Type']).filter(Boolean))];
        const uniqueLocations = [...new Set(allInternships.map(item => item['Location']).filter(Boolean))];
    ...
    const applyFilters = () => {
      ...
      if (filters.registerNo) {
        filtered = filtered.filter(item => 
          item['Register No']?.toLowerCase().includes(filters.registerNo.toLowerCase())
        );
      }
      ...
      if (filters.obtainedInternship) {
        filtered = filtered.filter(item => 
          item['Obtained Internship'] === filters.obtainedInternship
        );
      }
      
      if (filters.internshipType) {
        filtered = filtered.filter(item => 
          item['Internship Type'] === filters.internshipType
        );
      }
                        ...
                        <TableCell>
                          <Box sx={{ display: 'flex', gap: 0.5 }}>
                            <Tooltip title="Offer Letter">
                              <Chip 
                                label="OL" 
                                color={getStatusColor(internship['Offer Letter Submitted'])}
                                size="small"
                              />
                            </Tooltip>
                            <Tooltip title="Completion Certificate">
                              <Chip 
                                label="CC" 
                                color={getStatusColor(internship['Completion Certificate'])}
                                size="small"
                              />
                            </Tooltip>
                            <Tooltip title="Internship Report">
                              <Chip 
                                label="IR" 
                                color={getStatusColor(internship['Internship Report Submitted'])}
                                size="small"
                              />
                            </Tooltip>
                            <Tooltip title="Student Feedback">
                              <Chip 
                                label="SF" 
                                color={getStatusColor(internship['Student Feedback Submitted'])}
                                size="small"
                              />
                            </Tooltip>
                            <Tooltip title="Employer Feedback">
                              <Chip 
                                label="EF" 
                                color={getStatusColor(internship['Employer Feedback Submitted'])}
                                size="small"
                              />
                            </Tooltip>
                          </Box>
